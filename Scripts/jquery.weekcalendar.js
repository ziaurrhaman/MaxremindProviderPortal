        ...
        return {
            options: {
                ...
                */
                getEventUserId: function (calEvent, calendar) {
                    ...
                    //if (calEvent == undefined) return null;
            ...
            */
            _resizeCalendar: function () {
                ...
                /***Edited Section**/
                // Created By Ehtasham For Week View Width
                if (options.daysToShow == 7) {
                    $(".wc-container").css({ "overflow-y": "auto", "position": "relative" });
                    $(".wc-toolbar").css({ "position": "fixed", "width": "77.1% " });
                    $(".wc-header").css({ "margin-top": "24px", "width": "400%" });
                    $(".wc-scrollable-grid").css("width", "400%");
                }
                /***Edited Section**/
            },
              ...
              freeBusyManager;
                    ...
                    if (options.allowEventDelete && $target.hasClass('wc-cal-event-delete')) {
                        ...
                        $target.change(function () {
                            $calEvent.data('calEvent').statusId = $target.val();
                        });
                ...
                var $calEvent = $target.hasClass('wc-cal-event') ? $target : $target.parents('.wc-cal-event');
                var createdFromSingleClick = !$calEvent.hasClass('ui-resizable-resizing');
                        
                _calEventRightClick = $calEvent;
                _calendarRightClick = self.element;
            ...
            */
            ...
            $calendarBody, $calendarTableTbody;
                ...
                //rizwan
                $calendarBody = '<div onscroll="OnScrollAppointmentCalendar();" class=\"wc-scrollable-grid\">';
            ...
            */
            _setupEventCreationForWeekDay: function ($weekDay) {
                ...
                $weekDay.dblclick(function (event) {
                    var $target = $(event.target);
                    ...
                    if (event.button == 2) {
                        ...
                        SetCalEventCopyPaste($target);
                        //rizwan
                        $("[id$='divContextMenuAppointments']").css({
                            top: event.pageY - $(".contentWrapper").offset().top +90+ $(".contentWrapper").scrollTop() + "px",
                            left: event.pageX + 'px'

                           // 'top': e.pageY - 50, 'left': e.pageX, 'position': 'absolute', 'border': '1px solid black', 'padding': '5px'
                        ...
                        if ($newEvent.length) {
                            ...
                            _calEventRightClick = newCalEvent;
                            _calendarRightClick = self.element;
                            _selfRightClick = self;
                            _optionsRightClick = options;
                            _$weekDayRightClick = $weekDay;
            ...
            _addDroppableToWeekDay: function ($weekDay) {
                ...
                $weekDay.droppable({
                    ...
                    drop: function (event, ui) {
                        ...
                        var ifDropped = options.eventDrop(newCalEvent, calEvent, $calEvent);
            ...
            _addResizableToCalEvent: function (calEvent, $calEvent, $weekDay) {
                ...
                $calEvent.resizable({
                    ...
                    stop: function (event, ui) {
                        ...
                        UpdateAppointmentTime(calEvent.id, newEnd);
                    }
                });
            ...
            */
            _refreshEventDetails: function (calEvent, $calEvent) {
                ...
                if (!this.options.readonly) {
                    if (this.options.allowEventDelete && this.options.deletable(calEvent, $calEvent)) {
                        if (calEvent.PatientName == undefined) {
                            calEvent.PatientName = "";
                        }

                        //suffix_time = '<div class="event-date-time">' + this.options.eventHeader(calEvent, this.element) + "</div>";
                        suffix_time += '<div class="wc-cal-event-delete ui-icon ui-icon-close"></div>';
                        suffix_time += '<div class="patiant-name">';
                        suffix_time += '<span>' + calEvent.PatientName + '</span>';
                        suffix_time += '<input type="hidden" class="hdnEventId" value="' + calEvent.id + '" />';
                        suffix_time += '</div>';
                    }

                    suffix_time += '<div class="patient-info-icon-wrapper">';
                    suffix_time += '<span class="appointment-top-icon patient-info-icon" onclick="AppointmentViewQuickDetail(this, event, ' + calEvent.PatientId + ');"></span>';
                    suffix_time += '</div>';
                }

                //Start Code For Upadate Event Status With Warning WHEN Readonly is false and event allow update
                if (!this.options.readonly && this.options.allowEventUpdate) {
                    //                    suffix_title += '<div class="status-wrapper">';
                    //                    if (calEvent.end.getTime() > new Date().getTime()) {
                    //                        suffix_title += '<span class="ui-menu-icon ui-icon ui-icon-circle-triangle-e status-dropdown-icon" onclick="ShowHideAppointmentEventCustomDropDown(this, event, true);"></span>';
                    //                    }
                    //                    suffix_title += '<span class="status-label">';
                    //                    var appStatus = 'Pending';
                    //                    var appStatusDropDownHtml = '<div class="custom-dropdown-list">';
                    //                    appStatusDropDownHtml += '<table>';
                    //                    $("[id$='ddAppStatus'] option").each(function () {
                    //                        if (calEvent.statusId == $(this).val()) {
                    //                            appStatus = $(this).text();
                    //                        }
                    //                        appStatusDropDownHtml += '' +
                    //                        '<tr>' +
                    //                            '<td>' +
                    //                                '<span id="' + $(this).val() + '" onclick="updateAppointmentStatus(' + calEvent.id + ', this.id, event, this);">' + $(this).text() + '</span>' +
                    //                            '</td>' +
                    //                        '</tr>';
                    //                    });
                    //                    appStatusDropDownHtml += '</table>';
                    //                    appStatusDropDownHtml += '</div>';
                    //                    if (appStatus == "") {
                    //                        appStatus = appStatus + " (" + ")";
                    //                    }
                    //                    suffix_title += appStatus;
                    //                    suffix_title += '</span>';
                    //                    suffix_title += appStatusDropDownHtml;
                    //                    suffix_title += '</div>';
                }
                //End Code For Upadate Event Status With Warning WHEN Readonly is false and event allow update

                //Start Code For Upadate Event Status With Warning In Any Case
                //var isExpireTime = calEvent.end.getTime() < new Date().getTime();
                var isExpireTime = calEvent.end.getTime() < _PracticeLocationCurrentTimeUTC;
                //var isFutureEvent = calEvent.end.getTime() > new Date().getTime();

                var isFutureEvent = IsFutureDate(calEvent.start);

                suffix_title += '<div class="status-wrapper">' +
                '<div onclick="ShowHideAppointmentEventCustomDropDown(this, event, true);" > ' +
                    '<span class="status-dropdown-icon appointment-bottom-icon status-dropdown-icon-close"></span>' +
                    '<span class="status-label">';
                //'</div>';

                var appStatus = 'Pending';
                var appStatusDropDownHtml = '<div class="custom-dropdown-list" onmouseleave="onmouseLeaveStatusDDL();">';
                appStatusDropDownHtml += '<table>';
                ...
                $("[id$='ddAppStatus'] option").each(function () {
                    ...
                    appStatusDropDownHtml += '' +
                    '<tr>' +
                        '<td id="' + $(this).val() + '" onclick="updateAppointmentStatus(' + calEvent.id + ', this.id, event, this, ' + isExpireTime + ', ' + isFutureEvent + ', ' + calEvent.PatientId + ');">' +
                            '<span>' + $(this).text() + '</span>' +
                        '</td>' +
                    '</tr>';
                });
                ...
                suffix_title += '</div>';
                suffix_title += '</div>';
                //End Code For Upadate Event Status With Warning In Any Case

                if (calEvent.PatientId != 0) {
                    //Start Html For Eligibility
                    var objEligibilityStatus = ConvertPatientEligibilityStatus(calEvent.EligibilityStatus);
                    var CallFrom = 'Schedular';

                    suffix_title += '' +
                        '<div class="float-left-100" style="margin: 5px 0 0;">' +
                            '<div style="float: left;" onclick="ClickCheckPatientEligibility(this, event);">' +
                                '<span class="spnPatientEligibilityIcon appointment-bottom-icon ' + objEligibilityStatus.Css + '"></span>' +
                                '<span class="spnPatientEligibility font-small">' + objEligibilityStatus.EligibilityStatus + '</span>' +
                                '<input type="hidden" class="hdnPatientId" value="' + calEvent.PatientId + '" />' +
                            '</div>' +
                        '</div>';
                    //End Html For Eligibility
                }


                //Start Html For Reference No.
                suffix_title += '<div class="float-left-100" style="margin: 5px 0 0;">' +
                                    '<span class="spnBookingReferenceNo font-small">' + calEvent.BookingReferenceNo + '</span>' +
                                '</div>';
                //End Html For Reference No.

                $calEvent.find('.wc-time').html(suffix_time);
                $calEvent.find('.wc-title').html(this.options.eventBody(calEvent, this.element) + suffix_title);
