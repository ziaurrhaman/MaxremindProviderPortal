...
public partial class ProviderPortal_ReportsNew_filter_FilterPTLDetail : System.Web.UI.Page
{
    ...
    private void PatientFilterAll()
    {
        ...
        {
            ClaimId = int.Parse(Request.Form["ClaimId"]);
        }
        ...
        DataSet dsClaims = objClaimDB.GetClaimPatientptlList(Profile.PracticeId, PageNumber, Rows, PatientId, ClaimId, PatientName, PtlReason, PTLType, Payer, location, CommunicationCount, Dateofservice, LastCommuDate, QAApproved, AuditApproved, CRMApproved, DateFrom, DateTo, DateType);
        //Rizwan kharal End
        rptptlAll.DataSource = dsClaims.Tables[0];
        rptptlAll.DataBind();
        hdnTotalRows.Text = dsClaims.Tables[1].Rows[0]["TotalCount"].ToString();
